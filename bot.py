...
def health():
    return jsonify({
        "status": "ONLINE",
        "bot": "LC79 ELITE PRO MAX v5.0 VIP",
        "server": "Render",
        "algo": "QUANT-v5.0 SUPER ENGINE",
...
def BOX(title, body, color="blue"):
    """Giao diện Khung VIP v5 Ultra - Sang trọng, cân đối, siêu chuẩn mobile."""
...
# =========================
# TOÁN HỌC & SIÊU THUẬT TOÁN QUANT-v5.0 PRO
# =========================
def _entropy(arr):
    if not arr:
        return 0.0
...
def _pattern_detector(history):
    ...
    # Cầu 1-2-1 / 2-1-2
    if n >= 4:
        if h[-4] != h[-3] and h[-3] == h[-2] and h[-2] != h[-1]: # X T T X -> Tiếp theo T?
            return "1_2_1_pattern", h[-2], 72.0
        if h[-4] == h[-3] and h[-3] != h[-2] and h[-2] == h[-1]: # T T X T -> Tiếp theo X?
...
def super_predict(history, points_history=None, raw_hist=None):
    """
    SIÊU THUẬT TOÁN QUANT-v5.0 PRO
    Tổng hợp Multi-Engine: Pattern + Markov + Dice EMA + Entropy + Balance Bias
    """
    ...
    # 4. Entropy & Risk Warning
    ent = _entropy(history[-10:])
    risk = "🟢 SAFE"
    if ent > 0.95:
        risk = "⚠️ CAO (BIẾN ĐỘNG)"
    elif ent < 0.5:
        risk = "🔥 THẤP (CẦU RÕ)"

    # 5. Ma Trận Đa Đồng Thuận (Consensus Scoring)
    votes = {"TAI": 0.0, "XIU": 0.0}

    if pt_pred:
        votes[pt_pred] += (pt_conf / 100) * 0.40
    if mk_pred:
        votes[mk_pred] += (mk_conf / 100) * 0.30
    votes[ema_pred] += (ema_conf / 100) * 0.30

    final_pred = "TAI" if votes["TAI"] >= votes["XIU"] else "XIU"
    raw_confidence = max(votes["TAI"], votes["XIU"]) * 100
    final_confidence = min(96, max(58, int(raw_confidence)))

    dice_trend_str = f"EMA(5): {ema_val:.1f} → {'KÉO TÀI' if ema_val > 10.5 else 'ÉP XỈU'}"
    reason_str = f"Pattern: {pt_type} | Markov: {mk_pred or 'N/A'}"
    ...
    def connect():
        ...
        if arr:
            ...
            msg = BOX("🟢 KẾT NỐI VIP THÀNH CÔNG",
                      ...
                      f"🧠 Thuật toán Quant-v5.0 đã kích hoạt\n"
    ...
    try:
        ...
        if check_auth(cid):
            ...
            body = (
                ...
                f"⏳ THỜI HẠN: <b>{han}</b>\n"
                f"🧠 ALGO: <b>QUANT-v5.0 SUPER ENGINE</b>\n"
                f"──────────────\n"
                f"👉 Vui lòng chọn chức năng bên dưới menu:"
            )
            msg = BOX("👑 LC79 ELITE PRO MAX v5.0", body, "gold")
...
def lg(m):
    try:
        ...
        body = (
            ...
            f"🧠 SIÊU THUẬT TOÁN QUANT-v5.0 SẴN SÀNG"
        )
...
if __name__ == "__main__":
    try:
        ...
        logger.info("👑 LC79 ELITE PRO MAX v5.0 | QUANT ENGINE STARTED")
